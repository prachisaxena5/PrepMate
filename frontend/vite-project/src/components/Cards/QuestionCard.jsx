...
import { LuChevronDown, LuSparkles, LuStar, LuStarOff } from "react-icons/lu";
    ...
    <div className="bg-white rounded-lg mb-4 overflow-hidden py-4 px-5 shadow-xl shadow-gray-100/70 border border-gray-100/60 group">
      <div className="flex items-start justify-between cursor-pointer">
        <div className="flex items-start gap-3.5">
          ...
          <h3 className="text-xs md:text-[14px] font-medium text-gray-800 mr-0 md:mr-2" onClick={toggleExpand}>
            {question}
          </h3>
        </div>
        ...
        <div className="flex items-center justify-end ml-4 relative">
          <div className={`flex ${isExpanded ? "md:flex" : "md:hidden group-hover:flex"}`}>
            <button
              ...
              onClick={onTogglePin}
            >
              {isPinned ? <LuStarOff className="text-xs" /> : <LuStar className="text-xs" />}
            </button>
